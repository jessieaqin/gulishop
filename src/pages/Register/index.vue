<template>
  <div class="register-container">
    ...
    <div class="register">
      ...
      <div class="content">
        <label>确认密码:</label>
        <!-- <input type="text" placeholder="请输入确认密码" v-model="rpassword" />
        <span class="error-msg">错误提示信息</span> -->
        <input
          placeholder="请输入确认密码"
          v-model="rpassword"
...
export default {
  ...
  methods: {
    //登录收集信息
    async register() {
      //点击完成注册首先对所有的表单项做整体验证，验证通过返回的是true
      const success = await this.$validator.validateAll();

      //发送请求吧收集的数据，作为参数传递给后台存储数据库
      let { phone, code, password } = this;
      // if (phone && code && password && rpassword && rpassword == password) {
      if (success) {
        try {
          //注册成功
          this.$store.dispatch("getLogIn", { phone, code, password });
          //注册成功之后
          alert("注册成功,跳转登录页面");
